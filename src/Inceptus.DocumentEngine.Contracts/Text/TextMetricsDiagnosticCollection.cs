...
internal static class TextMetricsDiagnosticCollection
{
        ...
        string parameterName)
    {
        ...
        {
            throw new ArgumentException("Text-metrics diagnostics cannot contain null values.", parameterName);
        }
    ...
    private static IEqualityComparer<Diagnostic> Comparer { get; } = new DiagnosticValueComparer();
    ...
    private static int Compare(Diagnostic? left, Diagnostic? right)
    {
        ...
        comparison = comparison != 0 ? comparison : StringComparer.Ordinal.Compare(left.Message, right.Message);
        ...
        while (leftEnumerator.MoveNext())
        {
            ...
            comparison = comparison != 0
                ? comparison
                : StringComparer.Ordinal.Compare(leftEnumerator.Current.Value, rightEnumerator.Current.Value);
            if (comparison != 0)
            {
                return comparison;
            }
