...
/// <summary>
/// Immutable success or failure returned by an endpoint-reconnection factory.
/// </summary>
public sealed class ConnectorEndpointReconnectionPlanResult
{
    private ConnectorEndpointReconnectionPlanResult(
        ConnectorEndpointReconnectionPlan? plan,
        IEnumerable<Diagnostic>? diagnostics)
    {
        ...
        if (plan is not null && hasError)
        {
            throw new ArgumentException(
                "A successful endpoint-reconnection plan cannot contain error diagnostics.",
                nameof(diagnostics));
        }
        ...
        {
            throw new ArgumentException(
                "A failed endpoint-reconnection plan requires at least one error diagnostic.",
                nameof(diagnostics));
        }
    ...
    public ConnectorEndpointReconnectionPlan? Plan { get; }

    public ImmutableArray<Diagnostic> Diagnostics { get; }

    public static ConnectorEndpointReconnectionPlanResult Success(
        ConnectorEndpointReconnectionPlan plan,
    ...
    public static ConnectorEndpointReconnectionPlanResult Failure(
        IEnumerable<Diagnostic> diagnostics)
    {
        ...
        IEnumerable<Diagnostic>? diagnostics)
    {
        ...
        {
            throw new ArgumentException(
                "Endpoint-reconnection diagnostics cannot contain null values.",
                nameof(diagnostics));
        }
