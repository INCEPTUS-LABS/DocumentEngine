...
internal static class ProjectionDiagnosticCollection
{
        ...
        string parameterName)
    {
        ...
        if (Array.Exists(copy, static diagnostic => diagnostic is null))
        {
            throw new ArgumentException(
                "Projection diagnostics cannot contain null values.",
                parameterName);
        }
    ...
    private static int Compare(Diagnostic? left, Diagnostic? right)
    {
        ...
        return left.Context.SequenceCompare(right.Context);
    }
    ...
    private static int SequenceCompare(
        this IEnumerable<KeyValuePair<string, string>> left,
